...
def plot_multivariate_anomaly2(test_set, predictions,col,col_max_error, k):
    ...
    plt.savefig("plots_userSurge/user_surge_all_metric_plot2.png", dpi=200)
...
def reconstruction_error_plot2(test_set, predictions,col,results,threshold_line):
    ...
    tp_index_list = test_set.index[(test_set['is_anomaly'] == 1) & (test_set['predictions'] == 1)]
    fn_index_list = test_set.index[(test_set['is_anomaly'] == 0) & (test_set['predictions'] == 1)]
    tn_index_list = test_set.index[(test_set['is_anomaly'] == 1) & (test_set['predictions'] == 0)]

    x = (test_set['is_anomaly'] == 1) & (test_set['predictions'] == 1)
    y = (test_set['is_anomaly'] == 0) & (test_set['predictions'] == 1)
    z = (test_set['is_anomaly'] == 1) & (test_set['predictions'] == 0)
    a = (x==1) | (y==1)| (z==1)

    df_vals_tp = test_set.iloc[tp_index_list, -1]
    df_vals_tp_list = df_vals_tp.to_list()
    df_vals_tn = test_set.iloc[tn_index_list, -1]
    df_vals_tn_list = df_vals_tn.to_list()
    df_vals_fn = test_set.iloc[fn_index_list, -1]
    df_vals_fn_list = df_vals_fn.to_list()

    combined_list_tp = merge(tp_index_list, df_vals_tp_list)
    combined_list_tn = merge(tn_index_list, df_vals_tn_list)
    combined_list_fn = merge(fn_index_list, df_vals_fn_list)

    for element in combined_list_tp:
        plt.scatter([float(element.split('=')[0])], [float(element.split('=')[1])], c='red',
                    marker='|')  # , s=1000)
    for element in combined_list_tn:
    ...
    list_of_colums = ['col_1_list', 'col_2_list', 'col_3_list', 'col_4_list', 'col_5_list', 'col_6_list', 'col_7_list',
                      'col_8_list', 'col_9_list']
    df_vals_col = ['df_vals_col1','df_vals_col2','df_vals_col3','df_vals_col4','df_vals_col5','df_vals_col6','df_vals_col7','df_vals_col8','df_vals_col9']
    df_vals_col_list = ['df_vals_col1_list','df_vals_col2_list','df_vals_col3_list','df_vals_col4_list','df_vals_col5_list','df_vals_col6_list','df_vals_col7_list','df_vals_col8_list','df_vals_col9_list']
    combined_list_col = ['combined_list_col1','combined_list_col2','combined_list_col3','combined_list_col4','combined_list_col5','combined_list_col6','combined_list_col7','combined_list_col8','combined_list_col9']

    for i in range(9):
        list_of_colums[i] = test_set.index[(test_set.max_value == i) & (a == 1)]
        df_vals_col[i] = test_set.iloc[list_of_colums[i],-1]
        df_vals_col_list[i] = df_vals_col[i].values
        combined_list_col[i] = merge(list_of_colums[i], df_vals_col_list[i])
        if i == 0:
            for element in combined_list_col[0]:
    ...
    fig.tight_layout(pad=10.0)
    plt.savefig("plots_userSurge/user_surge_full_reconstruction_error_adjusted2.png", dpi=200)

def reconstruction_error_plot_for_nine_metrics(test_set, predictions,col,ax1,ax2,ax3,ax4,ax5,ax6,ax7,ax8,ax9, threshold):

    test_set["Index"] = range(test_set.shape[0])
    test_set['predictions'] = predictions
    test_set["max_value"] = col

    # test_set['max_value_error'] = max_value2
    # test_set['col_max_error'] = col_max_error
    ...
    l = ['ax1', 'ax2', 'ax3', 'ax4', 'ax5', 'ax6', 'ax7', 'ax8', 'ax9']
    # print(k)
    axs.plot(ax1, color='blue', linewidth=1)
    axs.plot(ax2, color='yellow', linewidth=1)
    axs.plot(ax3, color='purple', linewidth=1)
    axs.plot(ax4, color='brown', linewidth=1)
    axs.plot(ax5, color='limegreen', linewidth=1)
    axs.plot(ax6, color='gray', linewidth=1)
    axs.plot(ax7, color='olive', linewidth=1)
    axs.plot(ax8, color='crimson', linewidth=1)
    axs.plot(ax9, color='teal', linewidth=1)
    ...
    for element in combined_list_tp:
        axs.scatter([float(element.split('=')[0])],[-0.1], c='red',
                            marker='|')  # , s=1000)
    for element in combined_list_tn:
        axs.scatter([float(element.split('=')[0])],[-0.1], c='orange',
                            marker='|')  # , s=1000)
    for element in combined_list_fp:
        axs.scatter([float(element.split('=')[0])],[-0.1], c='green',
                            marker='|')  # , s=1000)

    col_name_list = ['col_1_list', 'col_2_list', 'col_3_list', 'col_4_list', 'col_5_list', 'col_6_list',
                         'col_7_list', 'col_8_list', 'col_9_list']
    df_vals_col = ['df_vals_col1', 'df_vals_col2', 'df_vals_col3', 'df_vals_col4', 'df_vals_col5', 'df_vals_col6',
                       'df_vals_col7', 'df_vals_col8', 'df_vals_col9']
    df_vals_col_list = ['df_vals_col1_list', 'df_vals_col2_list', 'df_vals_col3_list', 'df_vals_col4_list',
                            'df_vals_col5_list', 'df_vals_col6_list', 'df_vals_col7_list', 'df_vals_col8_list',
                            'df_vals_col9_list']
    combined_list_col = ['combined_list_col1', 'combined_list_col2', 'combined_list_col3', 'combined_list_col4',
                             'combined_list_col5', 'combined_list_col6', 'combined_list_col7', 'combined_list_col8',
                             'combined_list_col9']

    for i in range(9):
        col_name_list[i] = test_set.index[(test_set.max_value == i) & (a == 1)]
        df_vals_col[i] = test_set.iloc[col_name_list[i],-1]
        df_vals_col_list[i] = df_vals_col[i].values
        combined_list_col[i] = merge(col_name_list[i], df_vals_col_list[i])

        if i == 0:
            for element in combined_list_col[0]:
                axs.scatter([float(element.split('=')[0])], [-0.2], c='blue',
                            marker='|', s=50)

        elif i == 1:
            for element in combined_list_col[1]:
                axs.scatter([float(element.split('=')[0])], [-0.2], c='yellow',
                            marker='|', s=40)
        elif i == 2:
            for element in combined_list_col[2]:
                axs.scatter([float(element.split('=')[0])], [-0.2], c='purple',
                                    marker='|', s=40)
        elif i == 3:
            for element in combined_list_col[3]:
                axs.scatter([float(element.split('=')[0])], [-0.2], c='brown',
                                        marker='|', s=50)
        elif i == 4:
            for element in combined_list_col[4]:
                axs.scatter([float(element.split('=')[0])], [-0.2], c='limegreen',
                                    marker='|', s=40)
        elif i == 5:
            for element in combined_list_col[5]:
                axs.scatter([float(element.split('=')[0])], [-0.2], c='gray',
                                        marker='|', s=50)
        elif i == 6:
            for element in combined_list_col[6]:
                axs.scatter([float(element.split('=')[0])], [-0.2], c='olive',
                                        marker='|', s=40)
        elif i == 7:
            for element in combined_list_col[7]:
                axs.scatter([float(element.split('=')[0])], [-0.2], c='crimson',
                                        marker='|', s=50)
        elif i == 8:
            for element in combined_list_col[8]:
                axs.scatter([float(element.split('=')[0])], [-0.2], c='teal',
                                marker='|', s=50)
    ...
    fig.tight_layout(pad=10.0)
    plt.savefig("plots_userSurge/user_surge_reconstruction_error_all_metric.png", dpi=200)
