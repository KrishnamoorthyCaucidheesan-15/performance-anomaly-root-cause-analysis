...
from utils import get_default_device, to_device, plot_history, ROC,Evaluation,adjust_predictions_for_neighbourhood,metric_count_histogram,reconstruction_error_plot2,reconstruction_error_plot_for_nine_metrics,plot_multivariate_anomaly2
from usad import UsadModel, device,training, testing_individual_points, find_max_error,find_max_error_cause
...
#Read data
attack = pd.read_csv("Datasets/choreoDataset/choreo_labeled_anomaly_data/user_surge/choreo_for_usad_anomaly_slow_backend_user_surge_step.csv")
...
results,column_wise_results= testing_individual_points(model,test_loader)
print("column shape , " , column_wise_results.shape)
print(column_wise_results)
...
column_wise_results_list = results[0].tolist()
print(results[0])
...
# adjusted_predictions = evaluator
...
# fig.bar(y_pos, performance, align='center', alpha=0.5)
plt.xticks(y_pos, features)
plt.xticks(fontsize = 3)
plt.ylabel('Value')
plt.title('user_surge')
plt.savefig("plots_userSurge/user_surge_metric_count_histogram.png", dpi=200)

reconstructional_error_values = results[0].flatten().detach().cpu().numpy()
reconstructional_error_values_column = column_wise_results[0].flatten().detach().cpu().numpy()
# reconstructional_error_values_list = reconstructional_error_values.to_list()

# y_pred_for_ar = np.array(y_pred_for)
# plot_multivariate_anomaly(test_set, adjusted_predictions,col_maximum_error_index[0],maximum_deviating_metric[0], k)
plot_multivariate_anomaly2(test_set, adjusted_predictions,col_maximum_error_index[0],maximum_deviating_metric[0], k)
...
reconstruction_error_plot2(test_set,y_pred_for_arr,col_maximum_error_index[0],reconstructional_error_values,threshold)
